...
# 2. Tự động áp dụng giao diện Dark Mode
custom_style = """
<style>
...
def load_and_prepare_data(file_path):
    ...
    # Tự động phân loại Đại học / Cao đẳng an toàn
    if 'Link_Nguồn' in df.columns:
...
# 5. Thiết kế thanh Sidebar bên trái
with st.sidebar:
...
# 7. HIỂN THỊ DỮ LIỆU CHỐNG CRASH
st.title("🎓 HỆ THỐNG LỌC ĐIỂM CHUẨN")
st.markdown(f"Khối xét: **{selected_khoi}** | Điểm tối đa: **{diem_thi}** | Bậc: **{bac_dao_tao}**")
...
if len(filtered_df) > 0:
    ...
    # "CẦU CHÌ" BẢO VỆ MÁY CHỦ: Nếu quá 15 trường, cắt bớt để không sập web
    if len(school_list) > 15:
        st.warning(f"⚠️ Hiện đang có tới **{len(school_list)} trường** thỏa mãn. Để web chạy mượt và không bị đứng, hệ thống chỉ hiển thị **15 trường đầu tiên**. Hãy nhập thêm Tên trường hoặc hạ khoảng điểm xuống để xem cụ thể nhé!")
        school_list = school_list[:15]
